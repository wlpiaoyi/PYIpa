...
+ (instancetype)databaseWithPath:(NSString*)aPath {
    return PYFMDBReturnAutoreleased([[self alloc] initWithPath:aPath]);
}
...
- (void)dealloc {
    [self close];
    PYFMDBRelease(_openResultSets);
    PYFMDBRelease(_cachedStatements);
    PYFMDBRelease(_dateFormat);
    PYFMDBRelease(_databasePath);
    PYFMDBRelease(_openFunctions);
...
- (int)busyRetryTimeout {
    ...
    NSLog(@"PYFMDB: busyRetryTimeout no longer works, please use retryTimeout");
    return -1;
}

- (void)setBusyRetryTimeout:(int)i {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"PYFMDB: setBusyRetryTimeout does nothing, please use setRetryTimeout:");
}
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = PYFMDBReturnAutoreleased([_openResultSets copy]);
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = PYFMDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = PYFMDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    PYFMDBAutorelease(_dateFormat);
    _dateFormat = PYFMDBReturnRetained(format);
}
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    PYFMDBRetain(statement); // to balance the release below
...
void PYFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv);
void PYFMDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString*)name maximumArguments:(int)count withBlock:(void (^)(sqlite3_context *context, int argc, sqlite3_value **argv))block {
    ...
    id b = PYFMDBReturnAutoreleased([block copy]);
    
    [_openFunctions addObject:b];
    
    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &PYFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &PYFMDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
